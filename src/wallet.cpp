...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet) {
    ...
    // Проверка входных данных и защита от переполнения
    BOOST_FOREACH (const PAIRTYPE(CScript, int64)& s, vecSend) {
        if (s.second < 0) return false;
        if (nValue > std::numeric_limits<int64>::max() - s.second) return false; // Защита от переполнения
        nValue += s.second;
    }
...

    {
        ...
        // Фиксированная комиссия по вашему условию
        nFeeRet = 122523851; 
        ...
        // Проверка на переполнение при сложении суммы и комиссии
        if (nValue > std::numeric_limits<int64>::max() - nFeeRet) return false;
        int64 nTotalValue = nValue + nFeeRet;

        // 1. Наполняем выходы получателей
        BOOST_FOREACH (const PAIRTYPE(CScript, int64)& s, vecSend) {
            wtxNew.vout.push_back(CTxOut(s.second, s.first));
        }

        // 2. Ищем монеты под общую сумму
        set<pair<const CWalletTx*, unsigned int> > setCoins;
        int64 nValueIn = 0;
        if (!SelectCoins(nTotalValue, setCoins, nValueIn)) {
            printf("--- ОШИБКА: SelectCoins не нашел монеты под сумму %lld сатоши ---\n", nTotalValue);
            return false;
        }

        // 3. Расчет и добавление сдачи
        ...
        if (nChange > 0) {
            ...
            // Безопасное перемешивание выходов сдачи
            if (wtxNew.vout.empty()) {
                wtxNew.vout.push_back(CTxOut(nChange, scriptChange));
            } else {
                size_t nIndex = GetRandInt(wtxNew.vout.size() + 1);
                wtxNew.vout.insert(wtxNew.vout.begin() + nIndex, CTxOut(nChange, scriptChange));
            }
        } else {
            // Если сдачи нет, возвращаем ключ в пул
            reservekey.ReturnKey();
        }

        // 4. Заполняем входы (vin)
        ...
        BOOST_FOREACH(const PAIRTYPE(const CWalletTx*, unsigned int)& coin, setCoins) {
            ...
            wtxNew.vin[nIn].scriptSig.clear();

            if (!SignSignature(*this, scriptPubKey, wtxNew, nIn, 1)) {
                printf("--- КРИТИЧЕСКАЯ ОШИБКА: SignSignature вернул false для входа %d ---\n", nIn);
                return false;
            }
            nIn++;
        }

        // 6. Финализируем структуру в базе данных
